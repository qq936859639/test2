...
* 返回值   ：成功返回0，失败返回-1
***************************************/
int V4L::openVideo(const char *dev_name)
{
...
/**
 * @brief cameraThread::closeVideo
 * @param fd 摄像头文件 描述符
 * 关闭摄像头
 */
void V4L::closeVideo(int fd)
{
    ::close(fd);
}
/**************************************
...
* 返回值   ：成功返回0，失败返回-1
***************************************/
int V4L::requestVideoBufsAndMmap()
{
    unsigned int i = 0;
...
int V4L::storeImage()
{
    encode_jpeg((unsigned char*)(this->buffers[this->tV4L2buf.index].start)); //RGB24 to Jpeg
    return 0;
}

unsigned char *V4L::encode_jpeg(unsigned char *lpbuf)
{
    CLEAR(frame_buffer);
...
int V4L::yuyv_2_rgb888(unsigned char *buffers, unsigned char *frame_buffer)
{
    int           i,j;
