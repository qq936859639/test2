...
#include <QDebug>
FACES::FACES(){
    const char *model_path = "./faces/models";
...
Mat FACES::face_recognition(Mat img) {
    ...
    Mat img1;
    img.copyTo(img1);
//    char *szfile = "./faces/cyq.jpg";
//    getCurrentFilePath(img, saveFile);
    int Width = img.cols;
    int Height = img.rows;
    int Channels = img.channels();

//   unsigned char *inputImage = loadImage((char *)szfile, &Width, &Height, &Channels);
//   if (inputImage == nullptr || Channels != 3)
//    {
//        qDebug()<<"cjf 0";
//        return -1;
//    }
...
//    qDebug()<<"c:"<<(int)num_box<<",j:"<<img.type()<<",f:"<<img.step<<img.channels();
    printf("face num: %d \n", (int) num_box);
    ...
    for (int i = 0; i < num_box; i++) {
        ...
        Rect area(finalBbox[i].x1, finalBbox[i].y1, finalBbox[i].x2 - finalBbox[i].x1, finalBbox[i].y2 - finalBbox[i].y1);
        qDebug()<<finalBbox[i].x1<<"  "<<finalBbox[i].y1<<" "<<finalBbox[i].x2<<"  "<<finalBbox[i].y2;
        ...
        if (draw_face_feat) {
            ...
            drawRectangle(img.data, Width, Channels, finalBbox[i].x1, finalBbox[i].y1,
                          finalBbox[i].x2,
                          finalBbox[i].y2, red);
            const uint8_t blue[3] = {0, 0, 255};

            for (int num = 0; num < 5; num++) {
                drawPoint(img.data, Width, Channels, lround(finalBbox[i].ppoint[num]),
        ...
        RemoveRedEyes(img.data, img.data, Width, Height, Channels, left_eye_x, left_eye_y, radius);
        RemoveRedEyes(img.data, img.data, Width, Height, Channels, right_eye_x, right_eye_y, radius);
    }
    facialPoseCorrection(img.data, Width, Height, Channels, left_eye_x, left_eye_y, right_eye_x, right_eye_y);
//    saveImage("_done.jpg", Width, Height, Channels, ncnn_img);//img.data
